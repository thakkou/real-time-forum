1-ui
-update the time in messages ------////done
-update the toast to replace the others or at least 2 -----///done
-update the state online ,offline  //done
-add a toast in actions bete
-update the creat button ui //done

2-bugs
-when i recive a message and open a new  chat (not user) i get the message ----/////-----done 
-replace the offset by last id in feeds -----////---done
...
-search bar //done
-add nickname in toast notif ///done

check
-updaate the moda creat post (event + cursor to pointer)
/* ================================================================
   01FORUM — Brutalist Editorial CSS
   ================================================================ */
...
.btn.login:hover, .btn.chat:hover {
...
summary::-webkit-details-marker {
  display: none; /* removes arrow in Chrome, Safari, Edge */
}
...
.mobile-categories-grid input[type='checkbox'] {
  accent-color: var(--accent);
  width: 12px;
  height: 12px;
}
...
/* Post body — slightly more visible than before */
.post-body {
  padding: 1rem 1.5rem;
  color: #c8c8c8;
  font-size: 0.9rem;
...
.comment-actions .comment-dislike-btn {
  background: transparent;
  border: 1px solid var(--border);
  color: var(--text-dim);
  font-family: var(--mono);
...
.comment-actions .comment-dislike-btn:hover,
.comment-actions .comment-dislike-btn.active {
  border-color: var(--red);
  color: var(--red);
}
...
/* ── Auth Cards (login / register / error) ────────────────────── */
.card {
  width: 100%;
  max-width: 420px;
  margin: 6rem auto;
...
@media (max-width: 768px) {
  ...
  /* Show mobile sidebar inline in content */
  .sidebar-mobile {
    display: block;
  }
...
.create-post .categories label:hover {
  color: var(--text);
  border-color: var(--border-loud);
  background: rgba(232, 255, 71, 0.02);
}

.create-post .categories input[type='checkbox'] {
  accent-color: var(--accent);
  width: 12px;
  height: 12px;
  margin: 0;
...
.create-post .categories input[type='checkbox']:checked {
...
.create-post .categories label:has(input:checked) {
  color: var(--accent);
  border-color: var(--accent-dim);
  background: rgba(232, 255, 71, 0.04);
}
...
.image-preview.active .placeholder {
  display: none;
}
...
/* Optional: center preview on small screens */
@media (max-width: 480px) {
  .image-preview {
